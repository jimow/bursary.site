            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('financial_year') ? 'is-invalid' : '' }}" name="financial_year_id" id="financial_year_id">
                    @foreach($financial_years as $id => $entry)
                        <option value="{{ $id }}" {{ (old('financial_year_id') ? old('financial_year_id') : $systemSetting->financial_year->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\SystemSetting::SYSTEM_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('system_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="system_status_{{ $key }}" name="system_status" value="{{ $key }}" {{ old('system_status', $systemSetting->system_status) === (string) $key ? 'checked' : '' }}>
